...
| `prefs.controls_category.` | `prefs.controls_category.Keyboard Navigation` |
| `prefs.controls.`   | `prefs.controls.Key` |
| `prefs.command.`    | `prefs.command.Forward thrust` (описания из `Command.cpp`) |
| `info.`             | панель «Информация об игроке»: `info.player`, `info.net_worth`, `info.combat_rating`, `info.licenses` и т.д. |
| `info.column.`      | заголовки колонок флота: `info.column.ship`, `info.column.model` |
| `info.rating.`      | уровни рейтингов из `data/_ui/ratings.txt`: `info.rating.minimal`, `info.rating.sitting duck` |
| `info.license.`     | названия лицензий: `info.license.Pilot's` |
...
| `ru` | `FontSet::AddTtf(ttfPath, size, "ru")` | TTF из `%appdata%\roaming\endless sky\plugins\ru-data-translation\mainUI\fonts\` (имя берётся из `Preferences::Font()`, по умолчанию `ofont.ru_SouthGhetto.ttf`) |

Фактический путь в коде:
- `%appdata%\roaming\endless sky\plugins\ru-data-translation\mainUI\fonts\<fontFile>`
- где `<fontFile>` = `Preferences::Font()`, а если он пустой — `ofont.ru_SouthGhetto.ttf`.
