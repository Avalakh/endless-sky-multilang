	...
	string LocationName(OutfitterPanel::OutfitLocation location)
	{
		switch(location)
		{
			case OutfitterPanel::OutfitLocation::Ship:
				return Translation::Tr("ui.location_ship");
			case OutfitterPanel::OutfitLocation::Shop:
				return Translation::Tr("ui.location_shop");
			case OutfitterPanel::OutfitLocation::Cargo:
				return Translation::Tr("ui.location_cargo");
			case OutfitterPanel::OutfitLocation::Storage:
				return Translation::Tr("ui.location_storage");
			default:
...
void OutfitterPanel::DrawItem(const string &name, const Point &point)
{
	...
	if(playerShip || isLicense || mapSize)
	{
		...
		if(maxCount)
		{
			string label = Translation::Tr("ui.outfitter_installed") + to_string(minCount);
			Color color = bright;
			if(maxCount > minCount)
			{
				label += Translation::Tr("ui.outfitter_installed_sep") + to_string(maxCount);
	...
	if(cargo && storage && stock)
		message = Translation::Tr("ui.outfitter_cargo_stored") + to_string(cargo + storage) + ", " + Translation::Tr("ui.outfitter_in_stock") + to_string(stock);
	else if(cargo && storage)
		message = Translation::Tr("ui.outfitter_in_cargo") + to_string(cargo) + ", " + Translation::Tr("ui.outfitter_in_storage") + to_string(storage);
	else if(cargo && stock)
		message = Translation::Tr("ui.outfitter_in_cargo") + to_string(cargo) + ", " + Translation::Tr("ui.outfitter_in_stock") + to_string(stock);
	else if(storage && stock)
		message = Translation::Tr("ui.outfitter_in_storage") + to_string(storage) + ", " + Translation::Tr("ui.outfitter_in_stock") + to_string(stock);
	else if(cargo)
		message = Translation::Tr("ui.outfitter_in_cargo") + to_string(cargo);
	else if(storage)
		message = Translation::Tr("ui.outfitter_in_storage") + to_string(storage);
	else if(stock)
		message = Translation::Tr("ui.outfitter_in_stock") + to_string(stock);
	else if(!outfitter.Has(outfit))
		message = Translation::Tr("ui.outfitter_not_sold_here");
	if(!message.empty())
	{
...
double OutfitterPanel::DrawDetails(const Point &center)
{
	...
	if(selectedOutfit)
	{
		...
		selectedItem = selectedOutfit->TranslatedDisplayName();
...
void OutfitterPanel::DrawKey()
{
	...
	font.Draw(Translation::Tr("ui.outfitter_show_for_sale"), pos + labelOffset, color[showForSale]);
	...
	font.Draw(Translation::Tr("ui.outfitter_show_installed"), pos + labelOffset, color[showInstalled && playerShip]);
	AddZone(Rectangle(pos + checkboxOffset, activeAreaSize), [this]() { ToggleInstalled(); });

	pos.Y() += checkboxSize.Y();
	SpriteShader::Draw(box[showCargo], pos);
	font.Draw(Translation::Tr("ui.outfitter_show_cargo"), pos + labelOffset, color[showCargo]);
	AddZone(Rectangle(pos + checkboxOffset, activeAreaSize), [this](){ ToggleCargo(); });

	pos.Y() += checkboxSize.Y();
	SpriteShader::Draw(box[showStorage], pos);
	font.Draw(Translation::Tr("ui.outfitter_show_storage"), pos + labelOffset, color[showStorage]);
...
void OutfitterPanel::DrawOutfit(const Outfit &outfit, const Point &center, bool isSelected, bool isOwned) const
{
	...
	const string name = outfit.TranslatedDisplayName();
	const Font &font = FontSet::Get(14);
...
int OutfitterPanel::FindItem(const string &text) const
{
	...
	for(unsigned int i = 0; i < zones.size(); ++i, ++it)
	{
		...
		int index = Format::Search(outfit->TranslatedDisplayName(), text);
...
void OutfitterPanel::DrawButtons()
{
	...
	font.Draw(Translation::Tr("shop.you_have"), creditsPoint, dim);
	...
	font.Draw(Translation::Tr("shop.cargo_free"), cargoPoint, dim);
	...
	// Row 1.
	DrawButton(Translation::Tr("btn.buy"), Rectangle(Point(buttonCenterX + buttonOffsetX * -1, rowBaseY + rowOffsetY * 0), buttonSize),
		ButtonActive('b'), hoverButton == 'b', 'b');
	DrawButton(Translation::Tr("btn.install"), Rectangle(Point(buttonCenterX + buttonOffsetX * 0, rowBaseY + rowOffsetY * 0), buttonSize),
		ButtonActive('i'), hoverButton == 'i', 'i');
	DrawButton(Translation::Tr("btn.cargo"), Rectangle(Point(buttonCenterX + buttonOffsetX * 1, rowBaseY + rowOffsetY * 0), buttonSize),
		ButtonActive('c'), hoverButton == 'c', 'c');
	// Row 2.
	DrawButton(Translation::Tr("btn.sell"), Rectangle(Point(buttonCenterX + buttonOffsetX * -1, rowBaseY + rowOffsetY * 1), buttonSize),
		ButtonActive('s'), hoverButton == 's', 's');
	DrawButton(Translation::Tr(!CanMoveOutfit(OutfitLocation::Ship, OutfitLocation::Storage) &&
		CanMoveOutfit(OutfitLocation::Cargo, OutfitLocation::Storage) ? "btn.unload" : "btn.uninstall"),
		Rectangle(Point(buttonCenterX + buttonOffsetX * 0, rowBaseY + rowOffsetY * 1), buttonSize),
		ButtonActive('u'), hoverButton == 'u', 'u');
	DrawButton(Translation::Tr("btn.store"), Rectangle(Point(buttonCenterX + buttonOffsetX * 1, rowBaseY + rowOffsetY * 1), buttonSize),
		ButtonActive('r'), hoverButton == 'r', 'r');
	// Row 3.
	DrawButton(Translation::Tr("btn.leave"), Rectangle(Point(buttonCenterX + buttonOffsetX * 1, rowBaseY + rowOffsetY * 2), buttonSize),
