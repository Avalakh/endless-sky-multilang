...
void BankPanel::Draw()
{
	...
	table.Draw(Translation::Tr("bank.heading_type"));
	table.Draw(Translation::Tr("bank.heading_principal"));
	table.Draw(Translation::Tr("bank.heading_interest"));
	table.Draw(Translation::Tr("bank.heading_term"));
	table.Draw(Translation::Tr("bank.heading_payment"));
	table.Draw("");
	table.DrawGap(5);
	...
	for(const Mortgage &mortgage : player.Accounts().Mortgages())
	{
		...
		{
			table.Draw(Translation::Tr("bank.other"));
		...
		table.Draw(Translation::Tr("bank.pay_extra"));
		++row;
	...
	if(b.maintenanceCosts || maintenanceDue)
	{
		...
		table.Draw(Translation::Tr("bank.maintenance"));
		if(maintenanceDue)
		{
	...
	if(salariesIncome || tributeIncome || b.assetsReturns)
	{
		...
		static const char *const INCOME_KEYS[] = {"", "bank.income_salary", "bank.income_tribute", "bank.income_salary_tribute",
			"bank.income_assets", "bank.income_salary_assets", "bank.income_tribute_assets", "bank.income_salary_tribute_assets"};
		string incomeLabel = Translation::Tr(INCOME_KEYS[(salariesIncome != 0) + 2 * (tributeIncome != 0) + 4 * (b.assetsReturns != 0)]);
		const auto incomeLayout = Layout(310, Truncate::BACK);
		table.DrawCustom({incomeLabel, incomeLayout});
	...
	table.Advance(3);
	table.Draw(Translation::Tr("bank.total"), selected);
	table.Draw(Format::AbbreviatedNumber(totalPayment), unselected);
	table.Advance();

	// Draw the credit score.
	table.DrawAt(Point(0., FIRST_Y + 210.));
	string credit = Translation::Tr("bank.credit_score", {{"value", to_string(player.Accounts().CreditScore())}});
	...
	string amount;
	if(!qualify)
		amount = Translation::Tr("bank.qualify_no");
	else
		amount = Translation::Tr("bank.qualify_yes", {{"amount", Format::CreditString(qualify)}});
	...
	{
		table.Advance(4);
		table.Draw(Translation::Tr("bank.apply"), selected);
	}
