...
string Message::Text() const
{
	...
	Format::Expand(subs);
	return Command::ReplaceNamesWithKeys(Format::ReplaceTranslated(Phrase::ExpandPhrases(text), subs));
}
...
string Message::Text(const map<string, string> &subs) const
{
	...
	map<string, string> subsCopy = subs;
	return Command::ReplaceNamesWithKeys(Format::ReplaceTranslated(Phrase::ExpandPhrases(text), subsCopy));
}
