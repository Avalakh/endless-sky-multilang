...
void ItemInfoDisplay::UpdateDescription(const string &text, const vector<string> &licenses, bool isShip)
{
	...
	{
		string noun = Translation::Tr(isShip ? "ui.ship" : "ui.outfit");
		map<string, string> rep = {{"noun", noun}};
		string fullText = text + "\t" + Translation::Tr("ui.to_purchase_license", rep);
		fullText += Format::List<vector, string>(licenses,
			[](const string &name)
			{
				...
				return Translation::Tr(isVoweled ? "ui.license_an" : "ui.license_a") + name
					+ Translation::Tr("ui.license_suffix");
			});
		fullText += ".\n";
...
Point ItemInfoDisplay::Draw(Point point, const vector<string> &labels, const vector<string> &values,
	const vector<string> *tooltipKeys) const
{
	...
	const bool useTooltipKeys = tooltipKeys && tooltipKeys->size() == labels.size();
	for(unsigned i = 0; i < labels.size() && i < values.size(); ++i)
	{
		...
		CheckHover(table, useTooltipKeys ? (*tooltipKeys)[i] : labels[i]);
