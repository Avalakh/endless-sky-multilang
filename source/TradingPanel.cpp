...
namespace {
	const char *const TRADE_LEVEL_KEYS[] = {
		"trade.very_low",
		"trade.low",
		"trade.medium",
		"trade.high",
...
TradingPanel::~TradingPanel()
{
	if(profit)
	{
		string message = Translation::Tr("trade.sold") + " " + Format::CargoString(tonsSold, Translation::Tr("trade.cargo_unit"));

		if(profit < 0)
			message += " " + Translation::Tr("trade.loss", {{"amount", Format::CreditString(-profit)}});
		else
			message += " " + Translation::Tr("trade.profit_msg", {{"amount", Format::CreditString(profit)}});
...
void TradingPanel::Draw()
{
	...
	int y = FIRST_Y;
	font.Draw(Translation::Tr("Commodity"), Point(MIN_X + NAME_X, y), selected);
	font.Draw(Translation::Tr("Price"), Point(MIN_X + PRICE_X, y), selected);

	string mod = "x " + to_string(Modifier());
	font.Draw(mod, Point(MIN_X + BUY_X, y), unselected);
	font.Draw(mod, Point(MIN_X + SELL_X, y), unselected);

	font.Draw(Translation::Tr("In Hold"), Point(MIN_X + HOLD_X, y), selected);

	y += 5;
	int lastY = y + 20 * COMMODITY_COUNT + 25;
	font.Draw(Translation::Tr("trade.free"), Point(MIN_X + SELL_X + 5, lastY), selected);
	...
	if(player.Cargo().HasOutfits() || missionCargo)
	{
		...
		string str = Format::MassString(outfits + missionCargo) + Translation::Tr("trade.of");
		if(hasMinables && missionCargo)
			str += Translation::Tr("trade.cargo_mission_and_items");
		else if(hasOutfits && missionCargo)
			str += Translation::Tr("trade.cargo_outfits_and_mission");
		else if(hasOutfits && hasMinables)
			str += Translation::Tr("trade.cargo_outfits_and_commodities");
		else if(hasOutfits)
			str += Translation::Tr("trade.cargo_outfits");
		else if(hasMinables)
			str += Translation::Tr("trade.cargo_special_commodities");
		else
			str += Translation::Tr("trade.cargo_mission");
	...
	for(const Trade::Commodity &commodity : GameData::Commodities())
	{
		...
		string commodityDisplay = Translation::Tr("commodity." + commodity.name);
		if(commodityDisplay == "commodity." + commodity.name)
			commodityDisplay = commodity.name;
		font.Draw(commodityDisplay, Point(MIN_X + NAME_X, y), color);
		...
		if(price)
		{
			...
			font.Draw(Translation::Tr(TRADE_LEVEL_KEYS[level]), Point(MIN_X + LEVEL_X, y), color);

			font.Draw(Translation::Tr("[buy]"), Point(MIN_X + BUY_X, y), color);
			font.Draw(Translation::Tr("[sell]"), Point(MIN_X + SELL_X, y), color);
		}
		else
		{
			font.Draw("----", Point(MIN_X + PRICE_X, y), color);
			font.Draw(Translation::Tr("trade.not_for_sale"), Point(MIN_X + LEVEL_X, y), color);
		}
	...
	if(showProfit)
		font.Draw(Translation::Tr("Profit"), Point(MIN_X + PROFIT_X, FIRST_Y), selected);
