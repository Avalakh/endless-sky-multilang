	...
	else if(panelState.CanEdit() && key == 'D')
	{
		if(shipIt->get() != player.Flagship())
		{
			...
			if(!uniqueOutfits.empty())
			{
				...
				{
					message += "\n" + to_string(it->second) + " "
						+ (it->second == 1 ? it->first->TranslatedDisplayName() : it->first->TranslatedPluralName());
					++it;
				}
	...
	else if(key == 'c' && CanDump())
	{
		...
		else if(plunderAmount > 0 && selectedPlunder->Get("installable") < 0.)
		{
			GetUI().Push(DialogPanel::RequestPositiveInteger(this, &ShipInfoPanel::DumpPlunder,
				"How many tons of " + Format::LowerCase(selectedPlunder->TranslatedDisplayName())
		...
		else if(plunderAmount == 1)
		{
			GetUI().Push(DialogPanel::CallFunctionIfOk(this, &ShipInfoPanel::Dump,
				"Are you sure you want to jettison a " + selectedPlunder->TranslatedDisplayName() + "?"));
		}
		else if(plunderAmount > 1)
		{
			GetUI().Push(DialogPanel::RequestPositiveInteger(this, &ShipInfoPanel::DumpPlunder,
				"How many " + selectedPlunder->TranslatedPluralName() + " do you want to jettison?",
				plunderAmount));
		}
		else if(commodities)
		{
...
void ShipInfoPanel::DrawOutfits(const Rectangle &bounds, Rectangle &cargoBounds)
{
	...
	for(const auto &cat : GameData::GetCategory(CategoryType::OUTFIT))
	{
		...
		for(const Outfit *outfit : validOutfits)
		{
			...
			table.DrawTruncatedPair(outfit->TranslatedDisplayName(), dim,
...
void ShipInfoPanel::DrawWeapons(const Rectangle &bounds)
{
	...
	for(const Hardpoint &hardpoint : ship.Weapons())
	{
		...
		if(hardpoint.GetOutfit())
			name = hardpoint.GetOutfit()->TranslatedDisplayName();
	...
	if(draggingIndex >= 0)
	{
		...
		string name = outfit ? outfit->TranslatedDisplayName() : "[empty]";
...
void ShipInfoPanel::DrawCargo(const Rectangle &bounds)
{
	...
	if(cargo.HasOutfits() && hasSpace)
	{
		for(const auto &it : cargo.Outfits())
		{
			...
			string name = (isSingular ? it.first->TranslatedDisplayName() : it.first->TranslatedPluralName());
			if(!isSingular)
