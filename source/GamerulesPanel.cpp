...
namespace {
	// Gamerule translation keys (used for display and zone values).
	const string KEY_DEPRECIATION_MIN = "gamerules.depreciation_min";
	const string KEY_DEPRECIATION_GRACE_PERIOD = "gamerules.depreciation_grace_period";
	const string KEY_DEPRECIATION_MAX_AGE = "gamerules.depreciation_max_age";
	const string KEY_DEPRECIATION_DAILY = "gamerules.depreciation_daily";
	const string KEY_PERSON_SPAWN_PERIOD = "gamerules.person_spawn_period";
	const string KEY_NO_PERSON_SPAWN_WEIGHT = "gamerules.no_person_spawn_weight";
	const string KEY_NPC_MAX_MINING_TIME = "gamerules.npc_max_mining_time";
	const string KEY_UNIVERSAL_FRUGAL_THRESHOLD = "gamerules.universal_frugal_threshold";
	const string KEY_UNIVERSAL_RAMSCOOP = "gamerules.universal_ramscoop";
	const string KEY_SYSTEM_DEPARTURE_MIN = "gamerules.system_departure_min";
	const string KEY_SYSTEM_ARRIVAL_MIN = "gamerules.system_arrival_min";
	const string KEY_FLEET_MULTIPLIER = "gamerules.fleet_multiplier";
	const string KEY_LOCK_GAMERULES = "gamerules.lock_gamerules";
	const string KEY_FIGHTERS_HIT_WHEN_DISABLED = "gamerules.fighters_hit_when_disabled";
	const string KEY_UNIVERSAL_AMMO_STOCKING = "gamerules.universal_ammo_stocking";

	const string AMMO_RESTOCKING_NAME = "universal ammo restocking";

	const auto KEY_TO_RULE_NAME = map<string, string>{
		{KEY_DEPRECIATION_MIN, "depreciation min"},
		{KEY_DEPRECIATION_GRACE_PERIOD, "depreciation grace period"},
		{KEY_DEPRECIATION_MAX_AGE, "depreciation max age"},
		{KEY_DEPRECIATION_DAILY, "depreciation daily"},
		{KEY_PERSON_SPAWN_PERIOD, "person spawn period"},
		{KEY_NO_PERSON_SPAWN_WEIGHT, "no person spawn weight"},
		{KEY_NPC_MAX_MINING_TIME, "npc max mining time"},
		{KEY_UNIVERSAL_FRUGAL_THRESHOLD, "universal frugal threshold"},
		{KEY_UNIVERSAL_RAMSCOOP, "universal ramscoop"},
		{KEY_SYSTEM_DEPARTURE_MIN, "system departure min"},
		{KEY_SYSTEM_ARRIVAL_MIN, "system arrival min"},
		{KEY_FLEET_MULTIPLIER, "fleet multiplier"},
		{KEY_LOCK_GAMERULES, "lock gamerules"},
		{KEY_FIGHTERS_HIT_WHEN_DISABLED, "disabled fighters avoid projectiles"},
		{KEY_UNIVERSAL_AMMO_STOCKING, AMMO_RESTOCKING_NAME},
	};
...
bool GamerulesPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if(key == 'b' || command.Has(Command::MENU) || (key == 'w' && (mod & (KMOD_CTRL | KMOD_GUI))))
	{
		if(existingPilot && gamerules.LockGamerules())
			GetUI().Push(DialogPanel::CallFunctionIfOk([this]() -> void { GetUI().Pop(this); },
				Translation::Tr("gamerules.dialog_lock_confirm"), false));
		else
			GetUI().Pop(this);
	}
	else if(key == 'g' || key == 'p')
	...
	{
		const string &rule = KEY_TO_RULE_NAME.at(gameruleZones[latestIndex].Value());
...
void GamerulesPanel::DrawGamerules()
{
	...
	static const string GAMERULES[] = {
		"gamerules.category_depreciation",
		KEY_DEPRECIATION_MIN,
		KEY_DEPRECIATION_GRACE_PERIOD,
		KEY_DEPRECIATION_MAX_AGE,
		KEY_DEPRECIATION_DAILY,
		"",
		"gamerules.category_person_ships",
		KEY_PERSON_SPAWN_PERIOD,
		KEY_NO_PERSON_SPAWN_WEIGHT,
		"",
		"gamerules.category_npc_behavior",
		KEY_NPC_MAX_MINING_TIME,
		KEY_UNIVERSAL_FRUGAL_THRESHOLD,
		"\t",
		"gamerules.category_system_behavior",
		KEY_UNIVERSAL_RAMSCOOP,
		KEY_SYSTEM_ARRIVAL_MIN,
		KEY_SYSTEM_DEPARTURE_MIN,
		KEY_FLEET_MULTIPLIER,
		"",
		"gamerules.category_miscellaneous",
		KEY_LOCK_GAMERULES,
		KEY_FIGHTERS_HIT_WHEN_DISABLED,
		KEY_UNIVERSAL_AMMO_STOCKING
	};
	...
	for(const string &gamerule : GAMERULES)
	{
		...
		if(isCategory)
		{
			...
			table.Draw(Translation::Tr(gamerule), bright);
			table.Advance();
			table.DrawGap(5);
			continue;
		}
		...
		bool isOn = true;
		string text;
		if(gamerule == KEY_DEPRECIATION_MIN)
			text = Format::Percentage(gamerules.DepreciationMin(), 2);
		else if(gamerule == KEY_DEPRECIATION_GRACE_PERIOD)
			text = Format::AbbreviatedNumber(gamerules.DepreciationGracePeriod());
		else if(gamerule == KEY_DEPRECIATION_MAX_AGE)
			text = Format::AbbreviatedNumber(gamerules.DepreciationMaxAge());
		else if(gamerule == KEY_DEPRECIATION_DAILY)
			text = Format::Percentage(gamerules.DepreciationDaily(), 2);
		else if(gamerule == KEY_PERSON_SPAWN_PERIOD)
			text = Format::AbbreviatedNumber(gamerules.PersonSpawnPeriod());
		else if(gamerule == KEY_NO_PERSON_SPAWN_WEIGHT)
			text = Format::AbbreviatedNumber(gamerules.NoPersonSpawnWeight());
		else if(gamerule == KEY_NPC_MAX_MINING_TIME)
			text = Format::AbbreviatedNumber(gamerules.NPCMaxMiningTime());
		else if(gamerule == KEY_UNIVERSAL_FRUGAL_THRESHOLD)
			text = Format::Percentage(gamerules.UniversalFrugalThreshold(), 2);
		else if(gamerule == KEY_UNIVERSAL_RAMSCOOP)
			text = Translation::Tr(gamerules.UniversalRamscoopActive() ? "gamerules.value_true" : "gamerules.value_false");
		else if(gamerule == KEY_SYSTEM_ARRIVAL_MIN)
		{
			if(gamerules.SystemArrivalMin().has_value())
				text = Format::AbbreviatedNumber(*gamerules.SystemArrivalMin(), std::nullopt);
			else
				text = Translation::Tr("gamerules.value_unset");
		}
		else if(gamerule == KEY_SYSTEM_DEPARTURE_MIN)
			text = Format::AbbreviatedNumber(gamerules.SystemDepartureMin(), std::nullopt);
		else if(gamerule == KEY_FLEET_MULTIPLIER)
			text = Format::Percentage(gamerules.FleetMultiplier(), 2);
		else if(gamerule == KEY_LOCK_GAMERULES)
			text = Translation::Tr(gamerules.LockGamerules() ? "gamerules.value_true" : "gamerules.value_false");
		else if(gamerule == KEY_FIGHTERS_HIT_WHEN_DISABLED)
		{
			switch(gamerules.FightersHitWhenDisabled())
			{
			case Gamerules::FighterDodgePolicy::ALL:
				text = Translation::Tr("gamerules.fighter_policy_all");
				break;
			case Gamerules::FighterDodgePolicy::ONLY_PLAYER:
				text = Translation::Tr("gamerules.fighter_policy_player");
				break;
			case Gamerules::FighterDodgePolicy::NONE:
				text = Translation::Tr("gamerules.fighter_policy_none");
				break;
			}
		}
		else if(gamerule == KEY_UNIVERSAL_AMMO_STOCKING)
			text = Translation::Tr(gamerules.GetValue(AMMO_RESTOCKING_NAME) ? "gamerules.value_true" : "gamerules.value_false");
		...
		{
			auto width = FontSet::Get(14).Width(Translation::Tr(gamerule));
			table.SetHighlight(-120, width - 110);
			table.DrawHighlight(back);
		}

		table.Draw(Translation::Tr(gamerule), isOn ? medium : dim);
...
void GamerulesPanel::RenderPresetDescription(const Gamerules &preset)
{
	...
	wrap.Wrap(preset.Description().empty() ? Translation::Tr("gamerules.no_description") : preset.Description());
...
void GamerulesPanel::DrawTooltips()
{
	...
	if(!tooltip.HasText())
		tooltip.SetText(GameData::Tooltip(Translation::Tr(hoverItem)));
...
{
	if(str == KEY_DEPRECIATION_MIN)
	{
		string message = Translation::Tr("gamerules.msg_depreciation_min");
	...
	else if(str == KEY_DEPRECIATION_GRACE_PERIOD)
	{
		string message = Translation::Tr("gamerules.msg_depreciation_grace_period");
	...
	else if(str == KEY_DEPRECIATION_MAX_AGE)
	{
		string message = Translation::Tr("gamerules.msg_depreciation_max_age");
		auto validate = [](int value) -> bool { return value >= 0; };
	...
	else if(str == KEY_DEPRECIATION_DAILY)
	{
		string message = Translation::Tr("gamerules.msg_depreciation_daily");
		auto validate = [](double value) -> bool { return value >= 0.0 && value <= 1.0; };
	...
	else if(str == KEY_PERSON_SPAWN_PERIOD)
	{
		string message = Translation::Tr("gamerules.msg_person_spawn_period");
	...
	else if(str == KEY_NO_PERSON_SPAWN_WEIGHT)
	{
		string message = Translation::Tr("gamerules.msg_no_person_spawn_weight");
		auto validate = [](int value) -> bool { return value >= 0; };
	...
	else if(str == KEY_NPC_MAX_MINING_TIME)
	{
		string message = Translation::Tr("gamerules.msg_npc_max_mining_time");
		auto validate = [](int value) -> bool { return value >= 0; };
	...
	else if(str == KEY_UNIVERSAL_FRUGAL_THRESHOLD)
	{
		string message = Translation::Tr("gamerules.msg_universal_frugal_threshold");
		auto validate = [](double value) -> bool { return value >= 0.0 && value <= 1.0; };
	...
	else if(str == KEY_UNIVERSAL_RAMSCOOP)
		gamerules.SetUniversalRamscoopActive(!gamerules.UniversalRamscoopActive());
	else if(str == KEY_SYSTEM_DEPARTURE_MIN)
	{
		string message = Translation::Tr("gamerules.msg_system_departure_min");
	...
	else if(str == KEY_SYSTEM_ARRIVAL_MIN)
	{
		string message = Translation::Tr("gamerules.msg_system_arrival_min");
	...
	else if(str == KEY_FLEET_MULTIPLIER)
	{
		string message = Translation::Tr("gamerules.msg_fleet_multiplier");
		auto validate = [](double value) -> bool { return value >= 0.0; };
	...
	else if(str == KEY_LOCK_GAMERULES)
		gamerules.SetLockGamerules(!gamerules.LockGamerules());
	else if(str == KEY_FIGHTERS_HIT_WHEN_DISABLED)
	{
	...
	else if(str == KEY_UNIVERSAL_AMMO_STOCKING)
