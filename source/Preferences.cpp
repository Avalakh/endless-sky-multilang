...
namespace {
	...
	public:
		...
		const string &ToString() const
		{
			...
			switch(state)
			{
				case Preferences::OverlayState::OFF:     result = Translation::Tr("prefs.overlay_off"); break;
				case Preferences::OverlayState::ON:      result = Translation::Tr("prefs.overlay_always_on"); break;
				case Preferences::OverlayState::DAMAGED: result = Translation::Tr("prefs.overlay_damaged"); break;
				case Preferences::OverlayState::ON_HIT:  result = Translation::Tr("prefs.overlay_on_hit"); break;
				case Preferences::OverlayState::DISABLED: result = Translation::Tr("prefs.overlay_dash"); break;
			}
			return result;
		}
...
void Preferences::Load()
{
	...
	for(const DataNode &node : prefs)
	{
		...
		if(key == "window size" && node.Size() >= 3)
			...
			languageCode = node.Token(1);
		else if(key == "font" && hasValue)
			fontFilename = node.Token(1);
		else if(key == "letter spacing" && hasValue)
			letterSpacing = max(-5, min(10, static_cast<int>(node.Value(1))));
		else if(key == "font scale" && hasValue)
			fontScale = max(70, min(150, static_cast<int>(node.Value(1))));
#ifdef _WIN32
...
void Preferences::Save()
{
	...
	out.Write("language", languageCode);
	out.Write("font", fontFilename);
	out.Write("letter spacing", letterSpacing);
	out.Write("font scale", fontScale);
#ifdef _WIN32
...
void Preferences::SetLanguage(const string &code)
{
	languageCode = code.empty() ? "en" : code;
}
...
void Preferences::SetFont(const string &filename)
{
...
{
	if(Has(EXPEND_AMMO))
		return Has(FRUGAL_ESCORTS) ? Translation::Tr("prefs.ammo_frugally") : Translation::Tr("prefs.ammo_always");
	return Translation::Tr("prefs.ammo_never");
}
...
{
	static string result;
	result = dateFormatIndex == 0 ? Translation::Tr("prefs.date_dmy")
		: (dateFormatIndex == 1 ? Translation::Tr("prefs.date_mdy") : Translation::Tr("prefs.date_ymd"));
	return result;
}
...
{
	static string result;
	result = notifOptionsIndex == 0 ? Translation::Tr("prefs.notify_off")
		: (notifOptionsIndex == 1 ? Translation::Tr("prefs.notify_message") : Translation::Tr("prefs.notify_both"));
	return result;
}
...
void Preferences::SetLetterSpacing(int spacing)
{
	letterSpacing = max(-5, min(10, spacing));
}
...
void Preferences::SetFontScale(int percent)
{
	fontScale = max(70, min(150, percent));
}
...
{
	static string result;
	result = parallaxIndex == 0 ? Translation::Tr("prefs.parallax_off")
		: (parallaxIndex == 1 ? Translation::Tr("prefs.parallax_fancy") : Translation::Tr("prefs.parallax_fast"));
	return result;
}
...
{
	static string result;
	result = extendedJumpEffectIndex == 0 ? Translation::Tr("prefs.jump_effects_off")
		: (extendedJumpEffectIndex == 1 ? Translation::Tr("prefs.jump_effects_medium") : Translation::Tr("prefs.jump_effects_heavy"));
	return result;
}
...
{
	static string result;
	result = screenModeIndex == 0 ? Translation::Tr("prefs.screen_windowed") : Translation::Tr("prefs.screen_fullscreen");
	return result;
}
...
{
	static string result;
	result = vsyncIndex == 0 ? Translation::Tr("prefs.vsync_off")
		: (vsyncIndex == 1 ? Translation::Tr("prefs.vsync_on") : Translation::Tr("prefs.vsync_adaptive"));
	return result;
}
...
{
	static string result;
	result = cameraAccelerationIndex == 0 ? Translation::Tr("prefs.camera_off")
		: (cameraAccelerationIndex == 1 ? Translation::Tr("prefs.camera_on") : Translation::Tr("prefs.camera_reversed"));
	return result;
}
...
{
	static string result;
	result = turretOverlaysIndex == 0 ? Translation::Tr("prefs.turret_overlays_off")
		: (turretOverlaysIndex == 1 ? Translation::Tr("prefs.turret_overlays_always_on") : Translation::Tr("prefs.turret_overlays_blindspots"));
	return result;
}
...
{
	static string result;
	result = autoAimIndex == 0 ? Translation::Tr("prefs.auto_aim_off")
		: (autoAimIndex == 1 ? Translation::Tr("prefs.auto_aim_always_on") : Translation::Tr("prefs.auto_aim_when_firing"));
	return result;
}
...
{
	static string result;
	result = autoFireIndex == 0 ? Translation::Tr("prefs.auto_fire_off")
		: (autoFireIndex == 1 ? Translation::Tr("prefs.auto_fire_on")
		: (autoFireIndex == 2 ? Translation::Tr("prefs.auto_fire_guns_only") : Translation::Tr("prefs.auto_fire_turrets_only")));
	return result;
}
...
{
	static string result;
	result = boardingIndex == 0 ? Translation::Tr("prefs.boarding_proximity")
		: (boardingIndex == 1 ? Translation::Tr("prefs.boarding_value") : Translation::Tr("prefs.boarding_mixed"));
	return result;
}
...
{
	static string result;
	result = flotsamIndex == 0 ? Translation::Tr("prefs.flotsam_off")
		: (flotsamIndex == 1 ? Translation::Tr("prefs.flotsam_on")
		: (flotsamIndex == 2 ? Translation::Tr("prefs.flotsam_flagship_only") : Translation::Tr("prefs.flotsam_escorts_only")));
	return result;
}
...
{
	static string result;
	result = alertIndicatorIndex == 0 ? Translation::Tr("prefs.alert_off")
		: (alertIndicatorIndex == 1 ? Translation::Tr("prefs.alert_audio")
		: (alertIndicatorIndex == 2 ? Translation::Tr("prefs.alert_visual") : Translation::Tr("prefs.alert_both")));
	return result;
}
...
{
	static string result;
	result = minimapDisplayIndex == 0 ? Translation::Tr("prefs.minimap_off")
		: (minimapDisplayIndex == 1 ? Translation::Tr("prefs.minimap_when_jumping") : Translation::Tr("prefs.minimap_always_on"));
	return result;
}
...
{
	static string result;
	result = flagshipSpacePriorityIndex == 0 ? Translation::Tr("prefs.flagship_space_none")
		: (flagshipSpacePriorityIndex == 1 ? Translation::Tr("prefs.flagship_space_passengers")
		: (flagshipSpacePriorityIndex == 2 ? Translation::Tr("prefs.flagship_space_cargo") : Translation::Tr("prefs.flagship_space_both")));
	return result;
}
...
{
	static string result;
	result = largeGraphicsReductionIndex == 0 ? Translation::Tr("prefs.large_graphics_off")
		: (largeGraphicsReductionIndex == 1 ? Translation::Tr("prefs.large_graphics_largest_only") : Translation::Tr("prefs.large_graphics_all"));
	return result;
}
...
{
	static string result;
	result = titleBarThemeIndex == 0 ? Translation::Tr("prefs.title_bar_system")
		: (titleBarThemeIndex == 1 ? Translation::Tr("prefs.title_bar_light") : Translation::Tr("prefs.title_bar_dark"));
	return result;
}
...
{
	static string result;
	result = windowRoundingIndex == 0 ? Translation::Tr("prefs.window_rounding_system")
		: (windowRoundingIndex == 1 ? Translation::Tr("prefs.window_rounding_off")
		: (windowRoundingIndex == 2 ? Translation::Tr("prefs.window_rounding_large") : Translation::Tr("prefs.window_rounding_small")));
	return result;
}
#endif
