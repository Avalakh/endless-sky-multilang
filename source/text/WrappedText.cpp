...
void WrappedText::Wrap()
{
	...
	{
		// Use unsigned char so that UTF-8 high bytes (0x80–0xFF) are never
		// mistaken for whitespace. With signed char, values >= 0x80 become
		// negative and satisfy (c <= ' '), which silently drops all Cyrillic
		// (and any other non-ASCII) words, leaving the words vector empty.
		...
		// Otherwise, whitespace just adds to the x position.
			...
			word.x += Space(static_cast<char>(c));
