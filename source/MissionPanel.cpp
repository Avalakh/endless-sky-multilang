...
namespace {
	...
	// Try to translate a mission/job display name (e.g. "Bulk delivery to Hephaestus").
	// Uses keys like "job.Bulk delivery to" -> "Массовая доставка на " + destination.
	string TryTranslateMissionName(const string &name)
	{
		const struct { const char *sep; size_t sepLen; } patterns[] = {
			{" on ", 4},
			{" to ", 4},
			{" near ", 6},
			{" entering ", 10}
		...
		{
			size_t pos = name.rfind(p.sep);
			if(pos == string::npos)
				continue;
			string prefix = name.substr(0, pos);
			string suffix = name.substr(pos + p.sepLen);
			string key = "job." + prefix + string(p.sep, p.sepLen - 1);
			string translated = Translation::Tr(key);
			if(translated != key)
		...
		{
			key = "job." + name.substr(0, firstSpace);
			translated = Translation::Tr(key);
			if(translated != key)
				return translated + " " + name.substr(firstSpace + 1);
		}
		return name;
	}
...
void MissionPanel::Draw()
{
	...
	if(player.GetPlanet())
	{
		pos = DrawPanel(
			...
			Translation::Tr("mission.available_here"),
			available.size(),
			true);
	...
	pos = DrawPanel(
		...
		Translation::Tr("mission.your_current"),
		AcceptedVisible());
...
bool MissionPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if(key == 'A' || (key == 'a' && (mod & KMOD_SHIFT)))
	{
		if(acceptedIt != accepted.end() && acceptedIt->IsVisible())
			{
				map<string, string> rep = {{"name", acceptedIt->DisplayName()}};
				GetUI().Push(DialogPanel::CallFunctionIfOk(this, &MissionPanel::AbortMission,
					Translation::Tr("mission.abort_confirm", rep)));
			}
		return true;
	}
	...
	bool separateDeadlineOrPossible) const
{
	...
	for(auto it = missionList.begin(); it != missionList.end(); ++it)
	{
		...
		font.Draw({TryTranslateMissionName(it->DisplayName()), {SIDE_WIDTH - 11, Truncate::BACK}}, pos, *color);
	}
