...
void MainPanel::ShowScanDialog(const ShipEvent &event)
{
		...
		for(const auto &it : target->Cargo().Outfits())
			if(it.second)
			{
				...
				if(it.first->Get("installable") < 0.)
				{
					...
					out << Format::CargoString(tons, Format::LowerCase(it.first->TranslatedPluralName())) << "\n";
				}
				else
					out << it.second << " " << (it.second == 1 ? it.first->TranslatedDisplayName() : it.first->TranslatedPluralName()) << "\n";
			}
		if(first)
	...
	else if(event.Type() & ShipEvent::SCAN_OUTFITS)
	{
		...
		{
			string outfitNameForDisplay = (it.second == 1 ? it.first->TranslatedDisplayName() : it.first->TranslatedPluralName());
		...
		if(!count.empty())
		{
			...
			{
				const Ship *model = ships.Has(it.first) ? ships.Get(it.first) : nullptr;
				string fallbackDisplay = model ? model->DisplayModelName() : it.first;
				string fallbackPlural = model ? model->PluralModelName() : it.first + 's';
				string name = (it.second == 1)
					? Translation::TrShipName(it.first, fallbackDisplay)
					: Translation::TrShipPluralName(it.first, fallbackPlural);
				out << "\t" << it.second << " " << name << "\n";
			}
		}
	}
...
bool MainPanel::ShowHailPanel()
{
	...
	else if(target)
	{
		...
		if(target->Zoom() < 1. || target->IsDestroyed() || target->GetSystem() != player.GetSystem()
				|| target->IsCloaked())
			{
			map<string, string> rep = {{"noun", target->Noun()}};
			Messages::Add({Translation::Tr("message.unable_hail_target", rep), GameData::MessageCategories().Get("high")});
		}
		else if(target->IsEnteringHyperspace())
		{
			map<string, string> rep = {{"noun", target->Noun()}};
			Messages::Add({Translation::Tr("message.unable_hail_hyperspace", rep),
	...
	else if(flagship->GetTargetStellar())
	{
		...
		else if(planet->IsInhabited())
		{
			...
			{
				map<string, string> rep = {{"noun", planet->Noun()}};
				Messages::Add({Translation::Tr("message.unable_hail_not_inhabited", rep),
					GameData::MessageCategories().Get("high")});
			}
	}
	else
