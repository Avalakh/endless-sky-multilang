...
map<string, string> PlayerInfo::GetSubstitutions() const
{
	...
	Translation::TranslateSubstitutionValues(subs);
	return subs;
}
...
void PlayerInfo::AddPlayerSubstitutions(map<string, string> &subs) const
{
	...
	if(flag)
	{
		...
		subs["<model>"] = flag->TranslatedDisplayModelName();
		subs["<flagship>"] = flag->GivenName();
		subs["<flagship model>"] = flag->TranslatedDisplayModelName();
	}
...
void PlayerInfo::StepMissions(UI &ui)
{
	...
	if(flag)
	{
		...
		substitutions["<model>"] = flag->TranslatedDisplayModelName();
		substitutions["<flagship>"] = flag->GivenName();
		substitutions["<flagship model>"] = flag->TranslatedDisplayModelName();
	}
	...
	while(mit != missions.end())
	{
		...
		else if(mission.Destination() == GetPlanet() && !freshlyLoaded)
		{
			...
			if(visitText.empty())
			{
				...
				if(!text.empty())
					visitText = Format::ReplaceTranslated(text, substitutions);
			}
			++missionVisits;
		}
	}
...
void PlayerInfo::Fine(UI &ui)
{
	...
	if(!message.second.empty())
	{
		if(message.second == "atrocity")
		{
			...
			{
				message.second = "Before you can leave your ship, the " + Translation::TrGovernment(gov->DisplayName())
		...
		if(tributeIncome)
		{
			...
			if(!governments.empty())
			{
				message = "You have lost reputation with "
					+ Format::List(governments, [](const Government *gov){ return "the " + Translation::TrGovernment(gov->DisplayName()); })
