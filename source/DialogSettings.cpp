...
DialogSettings DialogSettings::Instantiate(const map<string, string> &subs) const
{
	...
	Translation::TranslateSubstitutionValues(subsCopy);
	string resultText;
	if(!text.empty())
		resultText = Format::Replace(Phrase::ExpandPhrases(text), subsCopy);
	else
		for(const DialogLine &line : lines)
		{
			...
			content = Format::Replace(Phrase::ExpandPhrases(content), subsCopy);
