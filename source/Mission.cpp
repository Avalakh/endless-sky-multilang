...
bool Mission::CanOffer(const PlayerInfo &player, const shared_ptr<Ship> &boardingShip) const
{
	...
	else if(location == ENTERING || location == TRANSITION)
	{
...
string Mission::BlockedMessage(const PlayerInfo &player)
{
	...
	string message = Format::ReplaceTranslated(blocked, subs);
	blocked.clear();
	return message;
}
	...
	if(event.TargetGovernment()->IsPlayer() && !IsFailed())
	{
		...
		else if(event.Type() & ShipEvent::BOARD)
		{
			...
			for(const auto &it : event.Actor()->Cargo().MissionCargo())
				failed |= (it.first == this);
			if(failed)
				message = "Your " + event.Target()->TranslatedDisplayModelName() +
...
Mission Mission::Instantiate(const PlayerInfo &player, const shared_ptr<Ship> &boardingShip) const
{
	...
	result.displayName = Format::ReplaceTranslated(Phrase::ExpandPhrases(displayName), subs);
	result.description = Format::ReplaceTranslated(Phrase::ExpandPhrases(
		Translation::TrMissionDescription(TrueName(), description)), subs);
	result.clearance = Format::ReplaceTranslated(Phrase::ExpandPhrases(clearance), subs);
	result.blocked = Format::ReplaceTranslated(Phrase::ExpandPhrases(blocked), subs);
