...
double ShipyardPanel::DrawDetails(const Point &center)
{
	...
	if(selectedShip)
	{
		...
		selectedItem = selectedShip->TranslatedDisplayModelName();
...
void ShipyardPanel::DoBuyButton()
{
	...
	if(modifier == 1)
		message += selectedShip->TranslatedDisplayModelName() + "! (Or leave it blank to use a randomly chosen name.)";
	else
		message += selectedShip->TranslatedPluralModelName() + "! (Or leave it blank to use randomly chosen names.)";
...
int ShipyardPanel::FindItem(const string &text) const
{
	...
	for(unsigned int i = 0; i < zones.size(); ++i, ++it)
	{
		...
		int index = Format::Search(ship->TranslatedDisplayModelName(), text);
