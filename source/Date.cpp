...
const string &Date::ToString() const
{
	...
	if(date && str.empty())
	{
		...
		static const string MONTH_KEY[] = {
				"date.month_jan", "date.month_feb", "date.month_mar", "date.month_apr",
				"date.month_may", "date.month_jun", "date.month_jul", "date.month_aug",
				"date.month_sep", "date.month_oct", "date.month_nov", "date.month_dec"};

		const string &monthStr = Translation::Tr(MONTH_KEY[month - 1]);
...
string Date::LongString() const
{
	...
	if(day / 10 == 1 || day % 10 == 0 || day % 10 > 3)
		dayString += Translation::Tr("date.ordinal_th");
	else if(day % 10 == 1)
		dayString += Translation::Tr("date.ordinal_st");
	else if(day % 10 == 2)
		dayString += Translation::Tr("date.ordinal_nd");
	else
		dayString += Translation::Tr("date.ordinal_rd");

	// Write out the month name instead of abbreviating it.
	static const string MONTH_KEY[] = {
		"date.month_january", "date.month_february", "date.month_march", "date.month_april",
		"date.month_may", "date.month_june", "date.month_july", "date.month_august",
		"date.month_september", "date.month_october", "date.month_november", "date.month_december"
	};
	const string &month = Translation::Tr(MONTH_KEY[Month() - 1]);
	...
	if(dateFormat == Preferences::DateFormat::YMD || dateFormat == Preferences::DateFormat::MDY)
		...
		result = Translation::Tr("date.ordinal_the") + dayString + Translation::Tr("date.ordinal_of") + month;
...
const string &Date::Weekday() const
{
	static const string DAY_KEY[] = {
		"date.weekday_sat", "date.weekday_sun", "date.weekday_mon", "date.weekday_tue",
		"date.weekday_wed", "date.weekday_thu", "date.weekday_fri"
	};
	static string result;
	result = Translation::Tr(DAY_KEY[WeekdayNumberOffset()]);
	return result;
}
