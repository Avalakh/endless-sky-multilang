...
namespace {
	...
	void SendMessage(const shared_ptr<const Ship> &ship, const string &message)
	{
		...
		string tag;
		const string &gov = Translation::TrGovernment(ship->GetGovernment()->DisplayName());
		if(!ship->GivenName().empty())
			tag = gov + " " + ship->Noun() + " \"" + ship->GivenName() + "\": ";
		else
			tag = ship->TranslatedDisplayModelName() + " (" + gov + "): ";
...
void Engine::Step(bool isActive)
{
	...
	else
	{
		...
		info.SetString("target type", target->TranslatedDisplayModelName());
		if(!target->GetGovernment())
			info.SetString("target government", Translation::Tr("ui.no_government"));
		else
			info.SetString("target government", Translation::TrGovernment(target->GetGovernment()->DisplayName()));
...
void Engine::EnterSystem()
{
	...
	{
		map<string, string> rep = {{"system", system->DisplayName()}, {"date", today.ToString()}};
		string msg = Translation::Tr("message.entering_system", rep);
		if(!system->IsInhabited(flagship))
			msg += Translation::Tr("message.no_inhabited_detected");
		Messages::Add({msg, GameData::MessageCategories().Get("daily")});
	}
		...
		if(attraction > 0.)
			for(int i = 0; i < 10; ++i)
				if(Random::Real() < attraction)
				{
					...
					{
					map<string, string> rep = {{"gov", Translation::TrGovernment(raidFleet.GetFleet()->GetGovernment()->DisplayName())}};
					Messages::Add({Translation::Tr("message.raid_attracted", rep),
				...
				if(planet->IsAccessible(flagship) && (clickPoint - position).Length() < object.Radius())
				{
					if(&object == flagship->GetTargetStellar())
					{
						if(!planet->CanLand(*flagship))
							{
							map<string, string> rep = {{"planet", planet->DisplayName()}};
							Messages::Add({Translation::Tr("message.authorities_refuse_land", rep),
								GameData::MessageCategories().Get("high")});
						}
...
void Engine::DoCollection(Flotsam &flotsam)
{
	...
	if(flotsam.OutfitType())
	{
		...
		{
			commodity = outfit->TranslatedDisplayName();
			player.Harvest(outfit);
		}
		else
			message = name + to_string(amount) + " "
				+ (amount == 1 ? outfit->TranslatedDisplayName() : outfit->TranslatedPluralName()) + ".";
	}
	else
