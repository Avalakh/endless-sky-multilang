...
bool Panel::DoHelp(const string &name, bool force) const
{
	...
	string titleKey = "help.title." + name;
	string title = Translation::Tr(titleKey);
	if(title == titleKey)
		title = Format::Capitalize(name);

	string bodyKey = "help." + name;
	string message = Translation::Tr(bodyKey);
	if(message == bodyKey)
		message = GameData::HelpMessage(name);
	if(message.empty())
		return false;

	Preferences::Set(preference);
	ui->Push(DialogPanel::Info(title + ":\n\n" + message));
