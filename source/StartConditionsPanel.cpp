...
void StartConditionsPanel::Draw()
{
			...
			++it, pos += Point(0., entryBox.Height()))
	{
		...
		const auto name = DisplayText(Translation::TrStartName(it->Identifier(), it->GetDisplayName()), Truncate::BACK);
...
void StartConditionsPanel::Select(StartConditionsList::iterator it)
{
	...
	info.SetString("name", Translation::TrStartName(startIt->Identifier(), startIt->GetDisplayName()));
	info.SetString("description", Translation::TrStartDescription(startIt->Identifier(), startIt->GetDescription()));
	...
	descriptionScroll = 0;
	description.Wrap(Translation::TrStartDescription(startIt->Identifier(), startIt->GetDescription()));
