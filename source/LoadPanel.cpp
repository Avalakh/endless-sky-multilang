...
void LoadPanel::Draw()
{
	...
	if(loadedInfo.IsLoaded())
	{
		...
		info.SetString("pilot", Translation::Tr("ui.no_pilot_loaded"));
...
bool LoadPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if(key == 'd' && !selectedPilot.empty())
	{
		...
		{
			map<string, string> rep = {{"name", loadedInfo.Name()}};
			GetUI().Push(DialogPanel::RequestStringWithValidation(this, &LoadPanel::DeletePilot,
				...
				Translation::Tr("dialog.delete_pilot_confirm", rep)));
		}
	}
	else if(key == 'a' && !player.IsDead() && player.IsLoaded())
	{
		...
		GetUI().Push(DialogPanel::RequestString(this, &LoadPanel::SnapshotCallback,
			Translation::Tr("dialog.snapshot_name_prompt"),
			FileDate(lastSave)));
	}
	else if(key == 'R' && !selectedFile.empty())
	{
		...
		if(!(fileName == selectedPilot + ".txt"))
			{
				map<string, string> rep = {{"path", selectedFile}};
				GetUI().Push(DialogPanel::CallFunctionIfOk(this, &LoadPanel::DeleteSave,
					Translation::Tr("dialog.delete_save_confirm", rep)));
			}
	}
	else if((key == 'l' || key == 'e') && !selectedPilot.empty())
	{
		...
		else
		{
			...
			GetUI().Push(DialogPanel::CallFunctionIfOk(this, &LoadPanel::LoadCallback,
				Translation::Tr("dialog.load_snapshot_overwrite")));
		}
	}
	else if(key == 'o')
...
void LoadPanel::SnapshotCallback(const string &name)
{
	...
	if(Files::Exists(to) && suffix != nameToConfirm)
	{
		...
		{
			map<string, string> rep = {{"name", suffix}};
			GetUI().Push(DialogPanel::RequestString(this, &LoadPanel::SnapshotCallback,
				Translation::Tr("dialog.warning_snapshot_overwrite", rep), suffix));
		}
	}
	else
...
void LoadPanel::WriteSnapshot(const filesystem::path &sourceFile, const filesystem::path &snapshotName)
{
	...
	if(Files::Copy(sourceFile, snapshotName))
	{
		...
		{
			map<string, string> rep = {{"path", snapshotName.string()}};
			GetUI().Push(DialogPanel::Info(Translation::Tr("dialog.error_create_file", rep)));
		}
}
...
void LoadPanel::DeletePilot(const string &)
{
	...
	if(failed)
		GetUI().Push(DialogPanel::Info(Translation::Tr("dialog.deleting_pilot_failed")));
...
void LoadPanel::DeleteSave()
{
	...
	if(Files::Exists(path))
		GetUI().Push(DialogPanel::Info(Translation::Tr("dialog.deleting_snapshot_failed")));
