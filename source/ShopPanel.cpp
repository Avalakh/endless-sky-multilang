		...
		else if(!Preferences::Has("help: shop with multiple ships"))
		{
			...
			for(const auto &it : player.Ships())
			{
				if(!CanShowInSidebar(*it, player.GetPlanet()))
					continue;
				if(modelNames.contains(it->TrueModelName()))
				{
					DoHelp("shop with multiple ships");
					break;
				}
				modelNames.insert(it->TrueModelName());
			}
		}
		checkedHelp = true;
	}
...
void ShopPanel::DrawShip(const Ship &ship, const Point &center, bool isSelected)
{
	...
	const string &name = ship.GivenName().empty() ? ship.TranslatedDisplayModelName() : ship.GivenName();
...
void ShopPanel::DrawShipsSidebar()
{
	...
	font.Draw({Translation::Tr("shop.your_ships"), {SIDEBAR_WIDTH, Alignment::CENTER}}, yoursPoint, bright);
...
void ShopPanel::DrawMain()
{
	...
	for(const auto &cat : categories)
	{
		...
		const string displayCategory = Translation::TrCategory(category);
		...
		if(!isEmpty)
		{
			Point size(bigFont.Width(displayCategory) + 25., bigFont.Height());
			...
			bigFont.Draw(displayCategory, side + Point(25., 0.), isCollapsed ? dim : bright);
