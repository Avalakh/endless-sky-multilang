...
void Files::Init(const char *const *argv)
{
	...
	// Directory of the main game executable (for paths relative to exe, e.g. data/fonts).
	{
		...
		if(basePath)
		{
			...
			SDL_free(basePath);
			if(Exists(executableDirectory))
				executableDirectory = filesystem::canonical(executableDirectory);
		}
	}
	...
	// Fonts: path "data/fonts" relative to the main game executable.
	if(executableDirectory.empty())
		fontsPath = dataPath / "fonts";
	else
	{
		const filesystem::path fontsRelativeToExe = executableDirectory / "data" / "fonts";
		fontsPath = Exists(fontsRelativeToExe) ? fontsRelativeToExe : (dataPath / "fonts");
	}
...
const filesystem::path &Files::ExecutableDirectory()
{
