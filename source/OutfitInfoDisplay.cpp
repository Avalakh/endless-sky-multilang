...
namespace {
	...
	const map<string, string> BOOLEAN_ATTRIBUTE_KEYS = {
		{"unplunderable", "outfit.unplunderable"},
		{"installable", "outfit.installable"},
		{"hyperdrive", "outfit.hyperdrive"},
		{"jump drive", "outfit.jump_drive"},
		{"minable", "outfit.minable"},
		{"map minables", "outfit.map_minables"},
		{"atrocity", "outfit.atrocity"},
		{"unique", "outfit.unique"},
		{"cloaked afterburner", "outfit.cloaked_afterburner"},
		{"cloaked boarding", "outfit.cloaked_boarding"},
		{"cloaked communication", "outfit.cloaked_communication"},
		{"cloaked deployment", "outfit.cloaked_deployment"},
		{"cloaked pickup", "outfit.cloaked_pickup"},
		{"cloaked scanning", "outfit.cloaked_scanning"}
	};
	...
	bool IsNotRequirement(const string &label)
	{
		return label == "automaton" ||
			...
			BOOLEAN_ATTRIBUTE_KEYS.find(label) != BOOLEAN_ATTRIBUTE_KEYS.end();
	}
}
...
{
	UpdateDescription(outfit.TranslatedDescription(), outfit.Licenses(), false);
...
{
	Draw(topLeft, requirementLabels, requirementValues, &requirementTooltipKeys);
}
		...
		bool canSell, bool descriptionCollapsed)
{
	...
	for(const string &license : outfit.Licenses())
	{
		...
		{
			requirementLabels.push_back(Translation::Tr("ship_info.license"));
			requirementTooltipKeys.push_back("license:");
	...
	if(buyValue == cost)
	{
		requirementLabels.push_back(Translation::Tr("outfit_info.cost"));
		requirementTooltipKeys.push_back("cost:");
	}
	else
	{
		map<string, string> rep = {{"pct", to_string((100 * buyValue) / cost)}};
		requirementLabels.push_back(Translation::Tr("ship_info.cost_pct", rep));
		requirementTooltipKeys.push_back("cost (%):");
	}
	...
	if(canSell && sellValue != buyValue)
	{
		if(sellValue == cost)
		{
			requirementLabels.push_back(Translation::Tr("outfit_info.sells_for"));
			requirementTooltipKeys.push_back("sells for:");
		}
		else
		{
			map<string, string> rep = {{"pct", to_string((100 * sellValue) / cost)}};
			requirementLabels.push_back(Translation::Tr("outfit_info.sells_for_pct", rep));
			requirementTooltipKeys.push_back("sells for (%):");
		}
	...
	if(outfit.Mass())
	{
		requirementLabels.emplace_back(Translation::Tr("outfit_info.mass"));
		requirementTooltipKeys.emplace_back("mass:");
	...
	requirementTooltipKeys.emplace_back();
...
void OutfitInfoDisplay::AddRequirementAttribute(string label, double value)
{
	...
	if(label == "required crew")
	{
		if(value > 0)
		{
			requirementLabels.push_back(Translation::Tr("outfit_info.required_crew"));
			requirementTooltipKeys.push_back("required crew:");
	...
	requirementLabels.push_back(label + Translation::Tr("outfit_info.needed"));
	requirementTooltipKeys.push_back(label + " needed:");
	requirementValues.push_back(Format::Number(-value));
	requirementsHeight += 20;
}
...
void OutfitInfoDisplay::UpdateAttributes(const Outfit &outfit)
{
	...
	for(const pair<const char *, double> &it : outfit.Attributes())
	{
		...
		auto bit = BOOLEAN_ATTRIBUTE_KEYS.find(it.first);
		if(bit != BOOLEAN_ATTRIBUTE_KEYS.end())
		{
			attributeLabels.emplace_back(Translation::Tr(bit->second));
	...
	if(weapon->Ammo())
	{
		...
		attributeValues.emplace_back(weapon->Ammo()->TranslatedDisplayName());
		attributesHeight += 20;
