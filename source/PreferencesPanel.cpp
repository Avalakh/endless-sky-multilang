...
namespace {
	...
	const string LANGUAGE_SETTING = "Language";
	const string FONT_SETTING = "Font";
	const string LETTER_SPACING_SETTING = "Letter spacing";
	const int LETTER_SPACING_MIN = -5;
	const int LETTER_SPACING_MAX = 10;
	const string FONT_SCALE_SETTING = "Font scale";
	const int FONT_SCALE_MIN = 70;
	const int FONT_SCALE_MAX = 150;
	const int FONT_SCALE_INCREMENT = 10;
	...
	string SettingLabel(const string &setting)
	{
		string key = "prefs.label." + setting;
...
bool PreferencesPanel::Scroll(double dx, double dy)
{
	...
	if(page == 's' && !hoverItem.empty())
	{
		...
		else if(hoverItem == LETTER_SPACING_SETTING)
		{
			int v = Preferences::LetterSpacing();
			if(dy < 0.)
				v = max(LETTER_SPACING_MIN, v - 1);
			else
				v = min(LETTER_SPACING_MAX, v + 1);
			Preferences::SetLetterSpacing(v);
		}
		else if(hoverItem == FONT_SCALE_SETTING)
		{
			int v = Preferences::FontScale();
			if(dy < 0.)
				v = max(FONT_SCALE_MIN, v - FONT_SCALE_INCREMENT);
			else
				v = min(FONT_SCALE_MAX, v + FONT_SCALE_INCREMENT);
			Preferences::SetFontScale(v);
		}
		return true;
	}
	else if(page == 'p')
...
void PreferencesPanel::DrawSettings()
{
	...
	for(const string &setting : SETTINGS)
	{
		...
		if(isCategory)
		{
			...
			table.DrawGap(10);
			table.DrawUnderline(medium);
			table.Draw(SettingLabel(setting), bright);
			table.Advance();
			table.DrawGap(5);
			continue;
		}
		...
		else if(setting == VSYNC_SETTING)
		{
			...
			isOn = Preferences::VSyncState() != Preferences::VSync::off;
		}
		else if(setting == STATUS_OVERLAYS_ALL)
		{
			text = Preferences::StatusOverlaysSetting(Preferences::OverlayType::ALL);
			isOn = Preferences::StatusOverlaysState(Preferences::OverlayType::ALL) != Preferences::OverlayState::OFF
				&& Preferences::StatusOverlaysState(Preferences::OverlayType::ALL) != Preferences::OverlayState::DISABLED;
		}
		else if(setting == CAMERA_ACCELERATION)
		{
			text = Preferences::CameraAccelerationSetting();
			isOn = Preferences::CameraAcceleration() != Preferences::CameraAccel::OFF;
		}
		else if(setting == LARGE_GRAPHICS_REDUCTION)
		{
			text = Preferences::LargeGraphicsReductionSetting();
			isOn = Preferences::GetLargeGraphicsReduction() != Preferences::LargeGraphicsReduction::OFF;
		}
		else if(setting == STATUS_OVERLAYS_FLAGSHIP)
		{
			text = Preferences::StatusOverlaysSetting(Preferences::OverlayType::FLAGSHIP);
			isOn = Preferences::StatusOverlaysState(Preferences::OverlayType::FLAGSHIP) != Preferences::OverlayState::OFF
				&& Preferences::StatusOverlaysState(Preferences::OverlayType::FLAGSHIP) != Preferences::OverlayState::DISABLED;
		}
		else if(setting == STATUS_OVERLAYS_ESCORT)
		{
			text = Preferences::StatusOverlaysSetting(Preferences::OverlayType::ESCORT);
			isOn = Preferences::StatusOverlaysState(Preferences::OverlayType::ESCORT) != Preferences::OverlayState::OFF
				&& Preferences::StatusOverlaysState(Preferences::OverlayType::ESCORT) != Preferences::OverlayState::DISABLED;
		}
		else if(setting == STATUS_OVERLAYS_ENEMY)
		{
			text = Preferences::StatusOverlaysSetting(Preferences::OverlayType::ENEMY);
			isOn = Preferences::StatusOverlaysState(Preferences::OverlayType::ENEMY) != Preferences::OverlayState::OFF
				&& Preferences::StatusOverlaysState(Preferences::OverlayType::ENEMY) != Preferences::OverlayState::DISABLED;
		}
		else if(setting == STATUS_OVERLAYS_NEUTRAL)
		{
			text = Preferences::StatusOverlaysSetting(Preferences::OverlayType::NEUTRAL);
			isOn = Preferences::StatusOverlaysState(Preferences::OverlayType::NEUTRAL) != Preferences::OverlayState::OFF
				&& Preferences::StatusOverlaysState(Preferences::OverlayType::NEUTRAL) != Preferences::OverlayState::DISABLED;
		}
		else if(setting == TURRET_OVERLAYS)
		{
			text = Preferences::TurretOverlaysSetting();
			isOn = Preferences::GetTurretOverlays() != Preferences::TurretOverlays::OFF;
		}
		else if(setting == CLOAK_OUTLINE)
		{
			text = Preferences::Has(CLOAK_OUTLINE) ? Translation::Tr("prefs.fancy") : Translation::Tr("prefs.fast");
			isOn = true;
		}
		else if(setting == AUTO_AIM_SETTING)
		{
			text = Preferences::AutoAimSetting();
			isOn = Preferences::GetAutoAim() != Preferences::AutoAim::OFF;
		}
		else if(setting == AUTO_FIRE_SETTING)
		{
			text = Preferences::AutoFireSetting();
			isOn = Preferences::GetAutoFire() != Preferences::AutoFire::OFF;
		}
		...
		else if(setting == LANGUAGE_SETTING)
		{
			isOn = true;
			string code = Preferences::Language();
			text = Translation::Tr("language.name_" + code);
			if(text == "language.name_" + code)
				text = code;
		}
		else if(setting == FONT_SETTING)
		{
			isOn = true;
			text = Preferences::Font().empty() ? "—" : Preferences::Font();
		}
		else if(setting == LETTER_SPACING_SETTING)
		{
			isOn = true;
			text = to_string(Preferences::LetterSpacing());
		}
		else if(setting == FONT_SCALE_SETTING)
		{
			isOn = true;
			text = to_string(Preferences::FontScale()) + "%";
		}
		else if(setting == NOTIFY_ON_DEST)
		{
			text = Preferences::NotificationSettingString();
			isOn = Preferences::GetNotificationSetting() != Preferences::NotificationSetting::OFF;
		}
		else if(setting == FLOTSAM_SETTING)
		{
			text = Preferences::FlotsamSetting();
			isOn = Preferences::GetFlotsamCollection() != Preferences::FlotsamCollection::OFF;
		}
		else if(setting == TURRET_TRACKING)
		{
			isOn = true;
			text = Preferences::Has(FOCUS_PREFERENCE) ? Translation::Tr("prefs.focused") : Translation::Tr("prefs.opportunistic");
		}
		else if(setting == FIGHTER_REPAIR)
		{
			isOn = true;
			text = Preferences::Has(FIGHTER_REPAIR) ? Translation::Tr("prefs.parallel") : Translation::Tr("prefs.series");
		}
		...
		{
			isOn = true;
			text = Preferences::Has(SHIP_OUTLINES) ? Translation::Tr("prefs.fancy") : Translation::Tr("prefs.fast");
		}
		else if(setting == HUD_SHIP_OUTLINES)
		{
			isOn = true;
			text = Preferences::Has(HUD_SHIP_OUTLINES) ? Translation::Tr("prefs.fancy") : Translation::Tr("prefs.fast");
		}
		...
		else if(setting == WINDOW_ROUNDING)
		{
			...
			text = isOn ? Translation::Tr("prefs.on") : Translation::Tr("prefs.off");
		...
		table.Draw(SettingLabel(setting), isOn ? medium : dim);
...
void PreferencesPanel::HandleSettingsString(const string &str, Point cursorPosition)
{
	...
	else if(str == LANGUAGE_SETTING)
	{
		vector<string> codes = Translation::AvailableLanguageCodes();
		if(!codes.empty())
		{
			size_t i = 0;
			for(; i < codes.size(); ++i)
				if(codes[i] == Preferences::Language())
					break;
			i = (i + 1) % codes.size();
			string next = codes[i];
			Preferences::SetLanguage(next);
			Translation::SetLanguage(next);
			if(next == "en")
				Preferences::SetLetterSpacing(1);
			else if(next == "ru")
				Preferences::SetLetterSpacing(-1);
		}
	}
	else if(str == FONT_SETTING)
		{
			if(!availableFonts.empty())
			{
				string current = Preferences::Font();
				size_t i = 0;
				if(!current.empty())
					for(; i < availableFonts.size(); ++i)
						if(availableFonts[i] == current)
							break;
				...
				if(std::filesystem::exists(fontPath))
				{
					...
					FontSet::AddTtf(fontPath, 18, "ru");
				}
			}
		}
	else if(str == LETTER_SPACING_SETTING)
		{
			int v = Preferences::LetterSpacing() + 1;
			if(v > LETTER_SPACING_MAX)
				v = LETTER_SPACING_MIN;
			Preferences::SetLetterSpacing(v);
		}
	else if(str == FONT_SCALE_SETTING)
		{
			int v = Preferences::FontScale() + FONT_SCALE_INCREMENT;
			if(v > FONT_SCALE_MAX)
				v = FONT_SCALE_MIN;
			Preferences::SetFontScale(v);
		}
