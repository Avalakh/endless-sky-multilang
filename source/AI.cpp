...
namespace {
	...
	void IssueDeploy(const PlayerInfo &player)
	{
		...
		if(!toDeploy.empty())
		{
			...
			size_t count = toDeploy.size();
			map<string, string> rep = {{"count", to_string(count)},
				{"noun", Translation::Tr(count == 1 ? "message.ship" : "message.ships")}};
			Messages::Add({Translation::Tr("message.deployed_carried", rep),
		...
		else if(!toRecall.empty())
		{
			...
			size_t count = toRecall.size();
			map<string, string> rep = {{"count", to_string(count)},
				{"noun", Translation::Tr(count == 1 ? "message.ship" : "message.ships")}};
			Messages::Add({Translation::Tr("message.recalled_carried", rep),
				GameData::MessageCategories().Get("normal")});
		}
	}
...
void AI::IssueFormationChange(PlayerInfo &player)
{
	...
	if(toSet)
	{
		rep["name"] = Translation::TrFormation(toSet->TrueName());
		message = Translation::Tr("message.ships_will_assume_formation", rep);
	}
	else
		message = Translation::Tr("message.ships_will_no_formation", rep);
...
void AI::IssueShipTarget(const shared_ptr<Ship> &target)
{
	...
	{
		map<string, string> rep = {{"name", target->GivenName()}};
		IssueOrder(newOrder, Translation::Tr(isEnemy ? "message.focusing_fire_on" : "message.following", rep));
	}
}
...
void AI::IssueAsteroidTarget(const shared_ptr<Minable> &targetAsteroid)
{
	...
	{
		map<string, string> rep = {{"name", targetAsteroid->DisplayName()}, {"noun", targetAsteroid->Noun()}};
		IssueOrder(newOrder, Translation::Tr("message.focusing_fire_asteroid", rep));
	}
}
...
void AI::IssueMoveTarget(const Point &target, const System *moveToSystem)
{
	...
	string description = Translation::Tr("message.moving_to_location");
	if(player.GetSystem() != moveToSystem)
	{
		map<string, string> rep = {{"system", moveToSystem->DisplayName()}};
		description += Translation::Tr("message.moving_to_system", rep);
	}
	else
		description += ".";
...
void AI::UpdateKeys(PlayerInfo &player, const Command &activeCommands)
{
	...
	if(activeCommands.Has(Command::FIGHT) && target && !target->IsYours() && !shift)
	{
		...
		{
			map<string, string> rep = {{"name", target->GivenName()}};
			IssueOrder(newOrder, Translation::Tr("message.focusing_fire_on", rep));
		}
	}
	...
	if(activeCommands.Has(Command::HOLD_FIRE) && !shift)
	{
		...
		IssueOrder(newOrder, Translation::Tr("message.holding_fire"));
	}
	if(activeCommands.Has(Command::HOLD_POSITION) && !shift)
	{
		OrderSingle newOrder{Orders::Types::HOLD_POSITION};
		IssueOrder(newOrder, Translation::Tr("message.holding_position"));
	}
	if(activeCommands.Has(Command::GATHER) && !shift)
	{
		...
		IssueOrder(newOrder, Translation::Tr("message.gathering_flagship"));
	}
...
void AI::DoAppeasing(const shared_ptr<Ship> &ship, double *threshold) const
{
	...
	*threshold = (1. - health) + .1;
	...
	if(language.empty() || player.Conditions().Get("language: " + language))
		Messages::Add({Translation::TrGovernment(government->DisplayName()) + " " + ship->Noun() + " \"" + ship->GivenName()
...
void AI::MovePlayer(Ship &ship, Command &activeCommands)
{
	...
	if(ship.IsEnteringHyperspace() && !ship.IsHyperspacing())
	{
		...
		{
			string missionLabel = Translation::Tr(missions == 1 ? "message.mission_requires" : "message.missions_require");
			string list = Format::List<set, const Planet *>(destinations,
				[](const Planet *const &planet)
				{
					return planet->DisplayName();
				});
			map<string, string> rep = {{"mission_label", missionLabel}, {"list", list}};
			Messages::Add({Translation::Tr("message.note_missions_require", rep),
	...
	else if(activeCommands.Has(Command::LAND) && !ship.IsEnteringHyperspace() && ship.Zoom() == 1.)
	{
		...
		else if(message.empty() && target && activeCommands.Has(Command::WAIT))
		{
			...
			{
				{
				map<string, string> rep = {{"noun", next->GetPlanet()->Noun()}};
				message = Translation::Tr("message.authorities_refuse_clear", rep);
			}
			...
			{
				map<string, string> rep = {{"name", next->DisplayName()}};
				message = Translation::Tr("message.switching_landing", rep);
			}
		}
		else if(message.empty())
		{
			...
			{
				{
				map<string, string> rep = {{"noun", target->GetPlanet()->Noun()}};
				message = Translation::Tr("message.authorities_refuse_clear", rep);
			}
				messageCategory = GameData::MessageCategories().Get("high");
				UI::PlaySound(UI::UISound::FAILURE);
			...
			else if(!types.empty())
			{
				...
				map<string, string> rep = {{"types", typesStr}, {"name", target->DisplayName()}};
				message = Translation::Tr("message.you_can_land_more", rep);
			}
			else
			{
				map<string, string> rep = {{"name", target->DisplayName()}};
				message = Translation::Tr("message.landing_on", rep);
			}
		}
		if(!message.empty())
	...
	else if(activeCommands.Has(Command::JUMP | Command::FLEET_JUMP))
	{
		...
		else if(isWormhole)
		{
			...
			{
			map<string, string> rep = {{"system", player.TravelPlan().back()->DisplayName()}};
			Messages::Add({Translation::Tr("message.wormhole_navigate", rep),
				GameData::MessageCategories().Get("normal")});
		}
		}
		if(ship.GetTargetSystem() && !isWormhole)
		{
			string name = Translation::Tr("message.selected_star");
			if(player.KnowsName(*ship.GetTargetSystem()))
				name = ship.GetTargetSystem()->DisplayName();

			map<string, string> rep = {{"name", name}};
			if(activeCommands.Has(Command::FLEET_JUMP))
			{
				...
				if(activeCommands.Has(Command::JUMP))
					Messages::Add({Translation::Tr("message.autopilot_jump_fleet", rep),
						GameData::MessageCategories().Get("normal")});
			}
			else
				Messages::Add({Translation::Tr("message.autopilot_jump", rep),
					GameData::MessageCategories().Get("normal")});
		}
	}
	...
	else if(activeCommands.Has(Command::HARVEST))
	{
		...
		IssueOrder(newOrder, Translation::Tr("message.preparing_harvest"));
	}
	...
	if(autoPilot.Has(Command::JUMP | Command::FLEET_JUMP) && !(player.HasTravelPlan() || ship.GetTargetSystem()))
	{
		...
		{
			{
				map<string, string> rep = {{"name", planet->DisplayName()}};
				Messages::Add({Translation::Tr("message.autopilot_landing_on", rep),
					GameData::MessageCategories().Get("normal")});
			}
...
void AI::IssueOrder(const OrderSingle &newOrder, const string &description)
{
	...
	if(selected.empty())
	{
		...
		who = (ships.empty() ? destroyedCount : ships.size()) > 1
			? Translation::Tr("message.your_fleet_is") : includeFlagship ? Translation::Tr("message.your_flagship_is") : Translation::Tr("message.your_escort_is");
	}
	else
	{
		...
		who = (ships.empty() ? destroyedCount : ships.size()) > 1
			? Translation::Tr("message.selected_escorts_are") : Translation::Tr("message.selected_escort_is");
	}
	if(ships.empty())
	{
		if(destroyedCount)
			Messages::Add({who + Translation::Tr("message.destroyed_unable_orders"),
				GameData::MessageCategories().Get("normal")});
		return;
	}
	...
	{
		if(!isValidTarget)
			Messages::Add({who + Translation::Tr("message.unable_no_longer") + description,
				GameData::MessageCategories().Get("normal")});
		else
			Messages::Add({who + Translation::Tr("message.no_longer") + description, GameData::MessageCategories().Get("normal")});
